...
public:
	...
	void uncompress(const string& filename)
	{
		...
		node cur = ht.Getroot();
		LongType filesize = cur->_weight._count;//源文件一共有多少个字节
		cout << "源文件中共有字节:" << filesize << endl;;
		size_t UnCompressCount = 0;//统计目前解压了多少字节
...
/*############################黑盒测试用例#########################################*/
//压缩txt文件
void CompressDocumentTest1()
{
	CompressFile cf;
	cf.Compress("file.txt");
	cf.uncompress("file.compress");
}
//压缩docx文件
void CompressDocumentTest2()
{
	CompressFile cf;
	cf.Compress("努力.docx");
	cf.uncompress("努力.compress");
}
//压缩图片
